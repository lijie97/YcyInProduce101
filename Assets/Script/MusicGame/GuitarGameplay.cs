...
public class GuitarGameplay : MonoBehaviour
{
	...
	public void StartPlaying( int playlistIndex )
	{
        ...
        mGame_uiManager.allScore = Player.Song.Notes.Count * 10;
	...
	protected IEnumerator ShowFireParticles( int stringIndex )
	{
        ...
        //Debug.Log(KeyboardControl.GetStringButton(stringIndex).transform.Find("hit").transform.name);
       
        KeyboardControl.GetStringButton(stringIndex).transform.Find("hit").GetComponent<ParticleSystem>().Play();
		SoundEffect.Play();
        ...
        yield return null;
        KeyboardControl.GetStringButton(stringIndex).transform.Find("hit").GetComponent<ParticleSystem>().Stop();
	...
	protected IEnumerator TrailHitRoutine( int noteIndex, GameObject trail )
	{
        ...
        KeyboardControl.GetStringButton(note.StringIndex).transform.Find("long note effect").GetComponent<ParticleSystem>().Play();
		KeyboardControl.GetStringButton(note.StringIndex).transform.Find("hit").GetComponent<ParticleSystem>().Play();
		//add sound effect
		SoundEffect.Play();
		
		//add aditional hit effect
		//KeyboardControl.GetStringButton(note.stringIndex).transform.Find("hit").GetComponent<ParticleSystem>().Play();
        ...
        KeyboardControl.GetStringButton(note.StringIndex).transform.Find("long note effect").GetComponent<ParticleSystem>().Stop();
	...
	protected bool IsInHitZone( GameObject note )
	{
        if (note.transform.position.z > 0.9f)  //未Miss
        {
            ...
            else if (note.transform.position.z > 1.72f && note.transform.position.z < 2f)  //great
            {
                ...
                combo += 1;
                great += 1;
                StartCoroutine(mGame_uiManager.DisplayText(string.Format("COMBO {0}", combo)));
	...
	protected GameObject CreateTrailObject( GameObject noteObject, Note note )
	{
		...
		//Scale the plane
		trail.transform.localScale = new Vector3( 0.08f, 1f, scaleZ );
        ...
        Material tempM = Resources.Load<Material>("MusicGame/Materials/trail_Materials");
        trail.GetComponent<MeshRenderer>().material = tempM;
        trail.GetComponent<Renderer>().material.shader = Shader.Find("Particles/Additive");
        trail.GetComponent<Renderer>().enabled = false;
		trail.name = "Trail";
		return trail;
	}
